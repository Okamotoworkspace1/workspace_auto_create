...
> **Mac で「Apple は…検証できませんでした」と出る場合**
>
> マルウェアが見つかったという意味ではなく、アプリに公証（notarization）が
> 付いていないという意味です。AivisSpeech は公式 FAQ で「迅速な配布を優先して
> いるため現時点ではコード署名を実施していない」と明記しており、想定どおりの挙動です。
>
> 1. 警告は「キャンセル」または「完了」で閉じる
> 2. **システム設定 → プライバシーとセキュリティ** を開く
> 3. 下の方の「"AivisSpeech"は開発元を確認できないため、使用がブロックされました」
>    の横にある **「このまま開く」** をクリック
>
> macOS 15 Sequoia 以降は、この手順しか通りません（「右クリック → 開く」は廃止）。
> 項目は一定時間で消えるので、**起動を試みた直後に**システム設定を開いてください。
>
> 実行前に、ダウンロード元が `aivis-project.com` か `github.com/Aivis-Project/` で
> あることをブラウザの履歴で確認してください。
>
> この操作を避けたい場合は、Mac にインストールが不要な `elevenlabs` / `fishaudio`、
> または Python パッケージとして入る `sbv2` / `gptsovits` を使ってください。
> jigoe は `[voice] engine` の 1 行でエンジンを切り替えられます。
